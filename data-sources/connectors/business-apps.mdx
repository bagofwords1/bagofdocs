...
description: "Connect Salesforce, NetSuite, ServiceNow, monday.com and PostHog so the agent can query them as tables"
icon: "boxes"
---
...
| [monday.com](#monday-com) | Board query specs over the GraphQL API | API token, or per-user OAuth | Beta |
...
<Note>
  The monday.com connector is in **beta**.
</Note>
...
**Sign in with monday.com — `oauth`** (scope: user, no fields). Each user signs in against `auth.monday.com` with read-only scopes (`boards:read workspaces:read users:read account:read`), so queries run with that person's own board permissions. This requires the `oauth_client_id` above to be configured on the connection. monday access tokens do not expire and no refresh token is issued.
