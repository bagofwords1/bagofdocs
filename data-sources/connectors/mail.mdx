...
<Warning>
  These connectors are **not** the [Email channel](/channels/email). The Email channel gives the Bag of words analyst _its own_ mailbox so it can send answers and receive questions — one shared address, configured once by an admin. These connectors do the opposite: they let the agent read _your_ mailbox as data. The two are configured separately and can be used together or independently.
</Warning>
...
- [OneNote](/data-sources/connectors/onenote) — read Microsoft OneNote notebooks with the same Entra app.
