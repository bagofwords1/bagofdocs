...
<AccordionGroup>
  ...
  <Accordion title="Code Standards" icon="file-code">
    ...
    **Scope**: All data sources
    ...
    ```
    Before starting any analysis, scan metadata in Git (dbt docs,
    markdown, schemas) and only then proceed with table selection.
    ```
  </Accordion>
  <Accordion title="Visualization Standards" icon="bar-chart-3">
    Guide how to visualize common metrics.

    ```
    For Sales metrics, default to a bar chart by category or time
    unless the user requests a different visualization.
    ```
  ...
  <Accordion title="Dashboard Layout" icon="layout-dashboard">
    Define dashboard composition and layout conventions.

    ```
    Place key KPIs at the top.
    Prefer bar/area charts for trends and breakdowns.
...
* **System** - program workflow/orchestration (agent behavior)
* **Visualization** - chart selection and visualization standards
* **Dashboard** - layout and UX conventions
