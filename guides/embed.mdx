...
<Note>
  ...
  ```python Python
  bow_url = input_bow_url.rstrip("/")
  if bow_url.lower().endswith("/api"):
      bow_url = bow_url[: -len("/api")]
  api_base = f"{bow_url}/api"
  ```
...
# Keep both server-side. You need them again at the callback.
session["bow_oauth_state"] = state
session["bow_oauth_verifier"] = verifier

authorize = f"{bow_url}/api/oauth/authorize?" + urlencode(
    {
        "response_type": "code",
        "client_id": client_id,
...
def create_report(api_base: str, access_token: str, data_source_ids: list[str]) -> dict:
    response = httpx.post(
        f"{api_base}/reports",
        headers={
...
def stream_completion(
    ...
    # timeout=None matters: a run can think for a long stretch between frames,
    # and httpx's default read timeout would abort the stream mid-answer.
    with httpx.Client(timeout=None) as client:
        with client.stream(
            "POST",
            f"{api_base}/reports/{quote(report_id, safe='')}/completions",
            headers={
                "Authorization": f"Bearer {access_token}",
                "Content-Type": "application/json",
                "Accept": "text/event-stream",
            ...
            json={
                "prompt": {"content": prompt, "mentions": [], "mode": "chat"},
                "stream": True,
            },
        ) as response:
            ...
            parse_sse(response.iter_lines(), on_event)
```
...
function createSseParser(onEvent: (event: BowSseEvent) => void) {
  ...
  function dispatch() {
    if (dataLines.length === 0) {
      // Comment-only frames carry no event. BOW sends ": ping" heartbeats
      // during quiet stretches of a run, and dispatching those would surface
      // a bogus malformed event on every heartbeat.
      eventName = "message";
      rawLines = [];
      return;
    }
...
def parse_sse(lines, on_event) -> None:
    event_name = "message"
    data_lines: list[str] = []
    raw_lines: list[str] = []
    ...
    def dispatch() -> None:
        ...
        if not data_lines:
            # Comment-only frames carry no event. BOW sends ": ping" heartbeats
            # during quiet stretches of a run, and dispatching those would
            # surface a bogus malformed event on every heartbeat.
            event_name = "message"
            raw_lines = []
            return
        ...
        if data_string == "[DONE]":
            ...
            try:
                ...
                on_event({
                    "event": event_name,
                    "payload": {"error": str(error), "data": data_string},
                    "raw": raw,
                    "received_at": received_at,
                    "malformed": True,
                })
